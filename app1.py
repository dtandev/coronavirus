...
generalDf.columns = ['Data', 'Potwierdzenia', 'Zgony', 'Wyleczeni', 'Hospitalizacje', 'Kwarantanna', 'Nadzór', 'Ilość testów']
...
coronaDfGeneral = coronaDfGeneral[(coronaDfGeneral["Timestamp"] > "2020-01-01") & (coronaDfGeneral["Timestamp"] < "2020-04-01")] # ta linijka czyści tylko dane z kilku błędnych pozycji - potem do wywalenia
...
preventionFig.add_trace(go.Scatter(x = generalDf['Data'], y = generalDf['Ilość testów'], mode ='lines+markers', name = 'Ilość testów'))

sunburstDf = sunburstDf[['Province', 'City', 'Infection/Death/Recovery', 'Timestamp']]
sunburstDf.columns = ['Województwo', 'Miasto', 'Przypadek', 'Ilość przypadków']
wojSunburstFig = px.sunburst(sunburstDf, path=['Województwo', 'Miasto', 'Przypadek'], values='Ilość przypadków',  color ='Ilość przypadków',color_continuous_scale='Reds',maxdepth=2, labels={"id":"Id", "parent":"Rodzic", 'labels':'Etykieta'})

wojSunburstFig.update_traces(
    marker = {"line": {"width": 1, 'color':'grey'}},
    hovertemplate='<b>%{label}<br><b>Ilość przypadków: %{value}',
    )
    ...
    dcc.Tabs(id="app-tabs", value= '1', colors =  {'border': '#d6d6d6', 'primary': '#1975F00', 'background': '#ffffff10'}, children=[
        ...
        dcc.Tab(label = 'Statystyki', value = '4' , children = html.Div([
            ...
            dbc.Row([
                dbc.Col([
                    dbc.Tabs([
                        dbc.Tab(label = 'Zachorowania', tab_id = "zachorowaniaTab"),
        ...
        dcc.Tab(label = 'Autorzy', value='6', children = html.Div([
            ...
            dbc.Row([
                dbc.Col([
                    ...
                    html.Div('za pomoc w przejciu przez meandry multi-indeksowania oraz prezentacji danych'),
                    ], md = 5),
            ...
            html.P('mkn.geosin@uwm.edu.pl'),
            html.Img(src = "http://uwm.edu.pl/geosin/wordpress/wp-content/uploads/2016/12/logo_pozmianie-e1546509093312.png", style={'width':'5%', 'margin-top':'15px', 'margin-bottom': '15px'}),
            html.H6('GeoSiN', style={'margin-top':'35px'}),
            html.H6('www.geosin.pl # www.facebook.com/mkn.geosin', style={'margin-top':'5px'}),
            html.Div()],
...
def mapPlot_tab_content(active_tab, value, endDate):
    ...
    if active_tab == 'zasiegwirusaTab':
        ...
        densityMap.update_traces(
            ...
            hovertemplate='%{customdata[0]}<br>Ilość zdarzeń:%{customdata[1]}'
        )
